...
// import { IdContext } from './ChatsBar/ChatsBar.jsx'; // Θα το αντικαταστήσουμε με το currentUser από το UserContext

// Initialize UserContext with a default shape that matches its expected value
export const UserContext = createContext({ currentUser: null, handleLogin: () => {}, handleLogout: () => {} });

function App() {
  const [currentUser, setCurrentUser] = useState(null); // Θα κρατάει το αντικείμενο του χρήστη {id, email, rank, ...} ή null
      ...
      if (email) {
        try {
          ...
          if (userId) {
            ...
            const profileResponse = await api.get(`/profile_info/${userId}`); // Αυτό το endpoint επιστρέφει rank
            ...
            setCurrentUser({
              ...
              rank: profileResponse.data.rank, // Σημαντικό: αποθήκευση του rank
            ...
            sessionStorage.removeItem('email'); // Καθαρισμός αν το email δεν αντιστοιχεί σε χρήστη
  ...
  const handleLogin = async (email, password) => {
    try {
      const loginResponse = await api.post('/login', { email, password }); // Το backend /api/login
  ...
  const handleLogout = () => {
    ...
    sessionStorage.removeItem('email');
    setCurrentUser(null);
    // Η πλοήγηση στην αρχική σελίδα θα γίνει από το Profile.jsx ή όπου καλείται το logout
  };
  ...
  return (
    <UserContext.Provider value={{ currentUser, handleLogin, handleLogout }}>
      <BrowserRouter>
        <Routes>
          <Route path="/" element={<Login />} /> {/* Το Login component θα χρειαστεί πρόσβαση στο handleLogin */}
          ...
          {/* Η διαδρομή για το Admin Panel θα είναι τώρα μέσα στο Home.jsx */}
        </Routes>
      </BrowserRouter>
